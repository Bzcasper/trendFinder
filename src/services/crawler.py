...
async def scrape_website(url: str, current_date: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        async with AsyncWebCrawler(config=browser_cfg) as crawler:
            ...
            if result.extracted_content and isinstance(result.extracted_content, dict) and "stories" in result.extracted_content:
                ...
                logger.info(f"Found {len(stories)} stories from {url}")
                ...
                logger.info(f"Scraped {url} in {elapsed_time:.2f} seconds")
                return {"stories": stories}
            else:
                # If no structured data was extracted, return empty stories
                logger.warning(f"No stories extracted from {url}")
                return {"stories": []}
    
    except Exception as e:
        # Classify and log the exception
        classified_error = classify_exception(e)
        logger.exception(f"Error scraping {url}: {str(classified_error)}")
        return {"stories": [], "error": str(classified_error)}

@retry_decorator(max_retries=3, initial_backoff=2.0)
async def scrape_twitter(url: str, bearer_token: str) -> Dict[str, Any]:
    """
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(api_url, headers=headers) as response:
                ...
                if tweets.get("meta", {}).get("result_count", 0) == 0:
                    logger.info(f"No tweets found for username {username}")
                    return {"stories": []}
                
                if not isinstance(tweets.get("data", []), list):
                    logger.error(f"Unexpected Twitter API response format")
                ...
                elapsed_time = time.time() - start_time
                logger.info(f"Found {len(stories)} tweets from {username} in {elapsed_time:.2f} seconds")
                return {"stories": stories}
    
    except Exception as e:
        # Classify and log the exception
        classified_error = classify_exception(e)
        logger.exception(f"Error fetching tweets for {username}: {str(classified_error)}")
        return {"stories": [], "error": str(classified_error)}

async def process_sources(sources: List[Dict[str, str]]) -> Dict[str, Any]:
    """
    ...
    if not twitter_token and twitter_sources:
        logger.warning("No X_API_BEARER_TOKEN found - Twitter sources will be skipped")
        twitter_sources = []
    ...
    logger.info(f"Processing {len(website_sources)} website sources and {len(twitter_sources)} Twitter sources")
    ...
    for future in asyncio.as_completed(all_tasks):
        try:
            ...
            if "error" in result:
                ...
                logger.error(f"Error in result from {source_url}: {result['error']}")
                errors.append({"source": source_url, "error": result["error"]})
            
            if "stories" in result and isinstance(result["stories"], list):
                all_stories.extend(result["stories"])
                logger.info(f"Added {len(result['stories'])} stories from {source['identifier']}")
        except Exception as e:
    ...
    logger.info(f"After processing: {len(processed_stories)} unique, relevant stories")
...
def main():
    ...
    if len(sys.argv) != 2:
        logger.error("Usage: python crawler.py '[{\"identifier\": \"https://example.com\", \"type\": \"website\"}]'")
        sys.exit(1)
